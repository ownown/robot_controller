...
int main(int argc, char **argv)
{
    ...
    while (rclcpp::ok())
    {
            ...
            rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            if (response.get()->left_motor_speed != left_motor_speed ||
                response.get()->right_motor_speed != right_motor_speed){
                    left_motor_speed = response.get()->left_motor_speed;
                    right_motor_speed = response.get()->right_motor_speed;
                    bp->set_motor_dps(LEFT_MOTOR, left_motor_speed);
                    bp->set_motor_dps(RIGHT_MOTOR, right_motor_speed);
                }
